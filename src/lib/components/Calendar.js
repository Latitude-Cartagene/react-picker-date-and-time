      ...
      <section className={classes.calendar}>
        <header className={classes.header}>
          <div className={classes.monthDisplay}>
            <img className={classes.previous} src={arrow} onClick={() => this.changeMonth('previous')} alt="previous" />
            {this.renderMonthLabel()}
            <img className={classes.next} src={arrow} onClick={() => this.changeMonth()} alt="next" />
          </div>
          <div className={classes.weekDays}>
            {this.state.days.map(day => {
              return (
                <span key={day} className={classes.weekDay}>
                  {day.charAt(0)}
                </span>
              );
            })}
